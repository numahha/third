    ...
    def reset_model(self):
        ...
        if self.friction_dist is not None: # add
            for i in range(0, len(self.sim.model.geom_friction)): # add
                self.sim.model.geom_friction[i,0] = self.default_geom_friction[i,0]*self.friction_dist() # add
                self.sim.model.geom_friction[i,1] = self.default_geom_friction[i,1]*self.friction_dist() # add
                self.sim.model.geom_friction[i,2] = self.default_geom_friction[i,2]*self.friction_dist() # add
