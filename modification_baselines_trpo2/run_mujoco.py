...
def train(args):
    ...
    custom_trpo_mpi.learn(env, policy_fn, timesteps_per_batch=args.batch_steps, episodes_per_batch=args.batch_episodes, 
                          max_kl=0.01, cg_iters=20, cg_damping=0.1,
                          max_iters=args.num_iters, max_timesteps=args.num_timesteps, 
                          gamma=0.995, lam=0.97, vf_iters=5, vf_stepsize=1e-3, input_dir=args.indir)
    env.close()
